...
- 恢复重启等待上限为 180 秒，健康 APK 安装上限为 120 秒，每次卸载上限为 60 秒；单次恢复最坏命令预算为 510 秒，低于 600 秒的遗留恢复收敛边界。残留应用清理只处理隔离记录中的 `quarantine_package_name`，不枚举或批量卸载其他第三方包。
...
- 设备页新增“恢复中”和“异常”不可用状态；不可用遮罩直接展示状态与原因，异常的固定两行摘要优先使用 `recovery_error`，鼠标悬停或键盘聚焦遮罩可检查未截断的完整恢复失败及其后保留的原始 `quarantine_reason`，遮罩后的操作按钮保持禁用。设备列表轮询调整为 60 秒。

本轮验证：

- 恢复与心跳聚焦测试 63 项通过；其中心跳测试 15 项，`quarantined`、`recovering`、`error` 均覆盖不探测、不写库且不修改快照的回归测试。
- 后端全量 unittest discovery 159 项通过。
- 前端新增 Node 内置测试，4 项全部通过，覆盖长恢复失败不截断、异常摘要优先级、原始隔离原因拼接以及恢复中/离线兜底文案。
...
## 2026-08-05（设备健康熔断与动态任务保护）

已完成设备健康保护方案并通过后端与前端集成验证：

- 心跳每 60 秒执行一次健康探测，检查 `adb get-state`、shell 标记、包管理器和 `/data` 至少 5 GiB 可用空间。
- 空闲设备心跳写入增加快照状态和空归属条件，探测期间被调度为 `busy` 的设备不会被旧探测结果覆盖。
- 新增持久化 `quarantined` 设备状态、隔离原因和时间；心跳不会自动恢复隔离设备。
- 新增设备完成工具初始化后立即执行完整健康探测，只有健康设备记录心跳并上线，低存储等异常设备直接隔离。
- 调度器只给最近 120 秒内健康心跳、状态为 `online` 且空闲的设备分配动态任务。
- 动态设备级错误在归属保护下原子化重新排队任务并隔离设备；安装后的 Phone Agent 强 ADB 异常及失败操作记录同样进入该流程，普通模型、存储和本地异常不误隔离设备。
- 每次动态执行使用独立 attempt 目录和对象存储命名空间，截图、日志、PCAP、动态文件和脱壳归档不会被并发重试覆盖或删除。
- 动态任务分发失败释放改为事务锁定任务/设备；忙碌设备可恢复在线，已隔离设备只清除旧归属并保持隔离。卸载失败时隔离设备并禁止释放。
- 旧 Worker 无法在设备或任务归属变化后写入结果、改变任务状态或释放新任务设备。
- 遵循明确规则：动态任务分配后、首次 APK 安装前不额外健康检查；模糊安装失败仅在安装失败后执行探测以便分类。
- 本轮不增加自动重启或自动恢复 Worker。

本轮验证：

- 后端全量 unittest discovery：通过。
