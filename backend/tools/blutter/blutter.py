...
def find_compat_macro(dart_version: str, no_analysis: bool):
    ...
    with open(os.path.join(vm_path, 'class_id.h'), 'rb') as f:
        ...
        if mm.find(b'V(LinkedHashMap)') != -1:
            ...
            # Dart 2.12 and older represented both maps and sets with the
            # LinkedHashMap VM class/cid.
            if mm.find(b'V(LinkedHashSet)') == -1:
                macros.append('-DOLD_MAP_SINGLE_CLASS=1')
    ...
    with open(os.path.join(vm_path, 'object.h'), 'rb') as f:
        mm = mmap.mmap(f.fileno(), 0, access = mmap.ACCESS_READ)
        # Before UntaggedObject was introduced, the raw object header type was
        # named ObjectLayout.
        if mm.find(b'class UntaggedObject') == -1:
            macros.append('-DPRE_UNTAGGED_OBJECT=1')
