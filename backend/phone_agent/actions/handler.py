...
class ActionHandler:
    ...
    def _handle_launch(self, action: dict, width: int, height: int) -> ActionResult:
        ...
        if not app_name and not package:
            return ActionResult(False, False, "No app  specified")

        device_factory = get_device_factory()
        if app_name:
            success = device_factory.launch_app(app_name, self.device_id)
        else:
            success = device_factory.launch_app_by_package(package, self.device_id)
        if success:
            return ActionResult(True, False)
        return ActionResult(False, False, f"App not found: {app_name}/{package}")
