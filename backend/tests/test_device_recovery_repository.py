...
class DeviceRecoveryRepositoryTest(unittest.TestCase):
    ...
    @patch("repositories.device_recovery_repo.uuid4", return_value="attempt-1")
    def test_claim_requires_idle_quarantined_device(
        self,
        _uuid4_mock,
        execute_mock,
        fetch_one_mock,
        ...
        self.assertIn("recovery_attempt_id = %s", sql)
        self.assertEqual(execute_mock.call_args.args[1], ("attempt-1", "device-1"))
        fetch_sql = fetch_one_mock.call_args.args[0]
        self.assertIn("recovery_attempt_id = %s", fetch_sql)
        self.assertEqual(fetch_one_mock.call_args.args[1], ("device-1", "attempt-1"))
    ...
    def test_success_requires_recovering_and_clears_isolation(self, execute_mock) -> None:
        ...
        self.assertTrue(complete_device_recovery("device-1", "attempt-1"))
    ...
    def test_failure_requires_recovering_and_sets_error(self, execute_mock) -> None:
        ...
        self.assertTrue(fail_device_recovery("device-1", "attempt-1", "reboot: timeout"))
        ...
        self.assertIn("last_recovery_at = NOW()", sql)
        self.assertIn("recovery_attempt_id = %s", sql)
        self.assertEqual(execute_mock.call_args.args[1], ("reboot: timeout", "device-1", "attempt-1"))
    ...
    def test_failure_error_is_clipped_to_2000_characters(self, execute_mock) -> None:
        ...
        fail_device_recovery("device-1", "attempt-1", "x" * 2001)

        self.assertEqual(len(execute_mock.call_args.args[1][0]), 2000)

    @patch("repositories.device_recovery_repo.execute", return_value=(0, 0))
    def test_stale_claimant_cannot_complete_a_newer_attempt(self, execute_mock) -> None:
        from repositories.device_recovery_repo import complete_device_recovery

        self.assertFalse(complete_device_recovery("device-1", "attempt-old"))

        sql = execute_mock.call_args.args[0]
        self.assertIn("status = 'recovering'", sql)
        self.assertIn("recovery_attempt_id = %s", sql)
        self.assertEqual(execute_mock.call_args.args[1], ("device-1", "attempt-old"))

    @patch("repositories.device_recovery_repo.execute", return_value=(0, 0))
    def test_stale_claimant_cannot_fail_a_newer_attempt(self, execute_mock) -> None:
        from repositories.device_recovery_repo import fail_device_recovery

        self.assertFalse(fail_device_recovery("device-1", "attempt-old", "reboot: timeout"))

        sql = execute_mock.call_args.args[0]
        self.assertIn("status = 'recovering'", sql)
        self.assertIn("recovery_attempt_id = %s", sql)
        self.assertEqual(
            execute_mock.call_args.args[1],
            ("reboot: timeout", "device-1", "attempt-old"),
        )
...
class DeviceProjectionRecoveryFieldsTest(unittest.TestCase):
    ...
    def test_new_device_initializes_recovery_fields(self, execute_mock) -> None:
        ...
        self.assertEqual(execute_mock.call_args.args[1][-6:], (None, None, None, None, None, None))
